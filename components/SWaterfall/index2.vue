...
export default {
  ...
  computed: {
    ...
    isMobile() {
      return !!navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i) // 初始化移动端*/
    },
  },
  mounted() {
    this.bindClickEvent()
  ...
  methods: {
    ...
    calcuCols() {
      // 列数初始化
      var waterfallWidth = this.width ? this.width : this.$refs.scrollEl.offsetWidth
    ...
    waterfall() {
      ...
      if (this.disableScroll) {
        const wfEls = document.querySelectorAll('.vue-waterfall-easy, .vue-waterfall-easy-scroll')
        for (let i = 0; i < wfEls.length; i++) {
          wfEls[i].style.height = Math.max.apply(null, this.colsHeightArr) + 'px'
        }
      }
    },
